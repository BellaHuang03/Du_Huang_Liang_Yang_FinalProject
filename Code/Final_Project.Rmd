...
```{r, Mainland USA, Wind Power Plant, 2023, echo=FALSE, message=FALSE, fig.cap="Figure 18. 2023 Wind Power Plants - Mainland USA. Power plants are shown as brown points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Alaska, Wind Power Plant, 2023, echo=FALSE, message=FALSE, fig.cap="Figure 19. 2023 Wind Power Plants - Alaska. Power plants are shown as brown points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Hawaii, Wind Power Plant, 2023, echo=FALSE, message=FALSE, fig.cap="Figure 20. 2023 Wind Power Plants - Hawaii. Power plants are shown as brown points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
#### Hydro Power
```{r, Mainland USA, Hydro Power Plant, 2018, echo=FALSE, message=FALSE, fig.cap="Figure 12. 2018 Hydro Power Plants - Mainland USA. Power plants are shown as dark blue points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Alaska, Hydro Power Plant, 2018, echo=FALSE, message=FALSE, fig.cap="Figure 13. 2018 Hydro Power Plants - Alaska. Power plants are shown as dark blue points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Hawaii, Hydro Power Plant, 2018, echo=FALSE, message=FALSE, fig.cap="Figure 14. 2018 Hydro Power Plants - Hawaii. Power plants are shown as dark blue points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Mainland USA, Hydro Power Plant, 2023, echo=FALSE, message=FALSE, fig.cap="Figure 21. 2023 Hydro Power Plants - Mainland USA. Power plants are shown as dark blue points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Alaska, Hydro Power Plant, 2023, echo=FALSE, message=FALSE, fig.cap="Figure 22. 2023 Hydro Power Plants - Alaska. Power plants are shown as dark blue points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Hawaii, Hydro Power Plant, 2023, echo=FALSE, message=FALSE, fig.cap="Figure 23. 2023 Hydro Power Plants - Hawaii. Power plants are shown as dark blue points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
#### Solar Power
```{r, Mainland USA, Solar Power Plant, 2018, echo=FALSE, message=FALSE, fig.cap="Figure 15. 2018 Solar Power Plants - Mainland USA. Power plants are shown as orange points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Alaska, Solar Power Plant, 2018, echo=FALSE, message=FALSE, fig.cap="Figure 16. 2018 Solar Power Plants - Alaska. Power plants are shown as orange points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, Hawaii, Solar Power Plant, 2018, echo=FALSE, message=FALSE, fig.cap="Figure 17. 2018 Solar Power Plants - Mainland USA. Power plants are shown as orange points. Data source: EPA eGRID (2018).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
The Mann–Kendall results presented in Table 2 reveal clear differences in long-term capacity trends across renewable technologies of Hydro, Solar, and Wind. Solar and wind both exhibit strong positive monotonic trends, with Kendall’s τ equal to 1.00 and statistically significant p-values (p = 0.0085), indicating there is a consistent increasing trend in installed capacity. In contrast, hydroelectric capacity shows a modest negative τ value (τ = –0.467), but its p-value (p = 0.2597) is not statistically significant, suggesting that there is not any statistically meaningful long-term trend. All together, solar and wind experienced clear and significant upward growth during this period, while hydroelectric capacity remained essentially stable due to geographic and infrastructural constraints.
...
```{r, TX, echo=FALSE, message=FALSE, fig.cap="Figure 27. Spatial Distribution of Texas Power Plants by Primary Fuel Type, 2023. Power plants are shown as colorful points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
```{r, CA, echo=FALSE, message=FALSE, fig.cap="Figure 29. Spatial Distribution of California Power Plants by Primary Fuel Type, 2023. Power plants are shown as colorful points. Data source: EPA eGRID (2023).", fig.align="center", fig.width=12, fig.height=6.5, out.width="100%"}
library(dplyr)
...
ggplot() +
 ...
 geom_sf(
  ...
  labs(
    title = "2018 Wind Power Plants - Washington",
    x = "Longitude",
    y = "Latitude",
    color = "Fuel Type"
  ) +
...
#12. prepare for state-year table
```{r}
...
```
#Subsection 3: By state: 1. Annual Capacity Trend (MW)
```{r}
...
```
#By state：Annual Generation Trend (MWh)
```{r}
...
```{r, echo=FALSE, message=FALSE, fig.cap="Graph 3. Three-Panel Trend Analysis of U.S. Wind Capacity (2018–2023). The panels show raw annual capacity values, LOESS-estimated trends with linear fits, and residual structure. Trend statistics (LM slope and Mann–Kendall τ) are displayed in the footer."}
...
summary_table %>%
  kable(
    format = "html",
    caption = "Table X. Summary of state-level regressions: absolute capacity growth (2018–2023) regressed on baseline (2018) capacity.",
    align = "c"
  ) %>%
...
This study examined spatial and temporal patterns of renewable energy development in the United States from 2018 to 2023, using plant-level data from the EPA’s eGRID database and associated geographic datasets. By integrating time-series trends, spatial distributions, statistical modeling, and state-level comparisons, the analysis provides a comprehensive view of how wind, solar, and hydroelectric technologies have evolved in recent years and how these patterns relate to broader questions about regional leadership, spatial clustering, and early-adopter momentum.

Across the national system, two technologies—solar and wind—show clear and statistically significant growth, while hydropower remains largely unchanged. Solar capacity increased at the fastest rate, expanding by approximately 12.7 GW per year, followed by wind at 10.8 GW per year, with both trends supported by strong linear and Mann–Kendall results. Hydroelectric capacity, in contrast, showed no meaningful expansion and instead exhibited a slight long-term decline of roughly 435 MW per year, consistent with its geographic and infrastructural constraints. Generation patterns further amplify these distinctions: wind and solar both display increasing annual production that aligns with capacity growth, while hydroelectric generation fluctuates sharply from year to year, driven primarily by hydrologic variability rather than new development.

Spatial analyses reveal that renewable expansion is highly geographically concentrated. Solar growth remains anchored in the Southwest and Southeast; wind continues to expand across the Great Plains and Texas; and hydroelectric facilities remain clustered in the Pacific Northwest and the Northeast. The Top-5 state rankings confirm stable technological “cores”: California for solar, Washington for hydropower, and Texas for wind. These states maintain their leadership positions throughout the study period, reflecting not only their resource advantages but also supportive policy environments and pre-existing infrastructure. Visualization of 2018 vs. 2023 spatial patterns further indicates that new renewable installations tend to reinforce existing geographic clusters rather than expand into new regions.

State-level case studies underscore the technology-specific drivers of these trends. Texas wind demonstrates rapid and consistent growth in plant count, capacity, and generation, supported by superior wind resources, large land availability, and major transmission investments such as the CREZ system. Washington hydro remains structurally stable, with no new facility development and year-to-year generation fluctuations strongly linked to drought cycles and snowpack variability. California solar continues to grow rapidly in both number of installations and installed capacity, driven by policy incentives, rooftop solar programs, and high solar irradiance, though generation grows slightly more slowly due to increasing curtailment and grid congestion. These cases illustrate how geographic, policy, and infrastructural factors shape technology-specific growth pathways.

Overall, the analysis demonstrates that the U.S. renewable energy landscape is characterized by strong technological divergence, persistent spatial clustering, and momentum effects favoring early adopters. Solar and wind show accelerating, geographically concentrated growth driven by resource potential and sustained investment, while hydropower remains stable due to physical and regulatory constraints. States that were already leaders in 2018—California, Texas, and Washington—continued to dominate their respective technologies, reinforcing regional specialization in the renewable energy sector. These results highlight not only the progress of the U.S. energy transition but also the structural patterns that will continue to shape renewable deployment in the coming decade.
\newpage
